...
public class Main {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        do {
            ...
            FileSize fileSize = FSFactory.createFileSize(file);
            do {
                if (fileSize.isDirectory()) {
                    fileSize = showMenuOptions(sc, fileSize);
                } else {
                    System.out.println("Displaying information for: " + fileSize.getName());
                    displayBytes(fileSize.getSize());
                }
            } while (!askPick(sc, fileSize));
        } while (!askQuit(sc));
    }
